 ...
 * @return void
 */
function ajax_inf_get_icons() {
	$icons = $_REQUEST['icons'];
	foreach ($icons as $icon) {
		$svgs[$icon] = inf_get_icon($icon);
	}

	wp_send_json_success($svgs);
}
add_action('wp_ajax_nopriv_inf_get_icons', 'ajax_inf_get_icons');
add_action('wp_ajax_inf_get_icons', 'ajax_inf_get_icons');
...
function inf_log($message, $payload = false, $log = 'admin') {
	...
	if (INF_LOG) {
		...
		$the_path = plugin_dir_path(__DIR__) . 'logs/';
