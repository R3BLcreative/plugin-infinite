...
class INF_Admin {
	...
	public function enqueue_styles() {
		wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/inf-admin.css', [], filemtime(plugin_dir_path(dirname(__FILE__)) . 'admin/css/inf-admin.css'), 'all');
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/inf-admin.js', [], filemtime(plugin_dir_path(dirname(__FILE__)) . 'admin/js/inf-admin.js'), ['in_footer' => true]);
	}
	...
	public function infinite_dashboard() {
		$screen = $this->get_current_screen();
		require_once plugin_dir_path(__FILE__) . 'partials/admin_dashboard.php';
	}
	...
	public function infinite_page() {
		...
		require_once plugin_dir_path(__FILE__) . 'partials/admin_page.php';
	}
	...
	public function infinite_header() {
		...
		require_once plugin_dir_path(__FILE__) . 'partials/temp_header.php';
	}
	...
	public function infinite_nav() {
		...
		if ($views) {
			...
			require_once plugin_dir_path(__FILE__) . 'partials/temp_nav.php';
		}
	}
	...
	public function infinite_pagination($total, $pages) {
		...
		if ($pages > 1) {
			require_once plugin_dir_path(__FILE__) . 'partials/comp_pagination.php';
		}
	}

	/**
	 * Display data bar content
	 *
	 * @since    1.0.8
	 */
	public function infinite_data_bar($slug, $data = []) {
		$screen = $this->get_current_screen();
		$view = (isset($_GET['view'])) ? $_GET['view'] : false;

		require_once plugin_dir_path(__FILE__) . 'partials/' . $slug . '.php';
	}
	 ...
	 * @since    1.0.0
	 */
	public function infinite_filters_bar($data) {
		$screen = $this->get_current_screen();
		$current_view = $this->get_current_view();
		$view = $current_view->slug;

		require_once plugin_dir_path(__FILE__) . 'partials/comp_filters.php';
	}

	/**
	 * Display bulk actions bar bar
	 *
	 * @since    1.0.8
	 */
	public function infinite_bulk_bar($data) {
		$screen = $this->get_current_screen();
		$current_view = $this->get_current_view();
		$view = $current_view->slug;

		require_once plugin_dir_path(__FILE__) . 'partials/comp_bulk.php';
	}
